...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export default function doInserts(bookshelf: AnyBookshelf): Promise<any> {
  ...
  return Promise.all([
    ...
    knex('sitesmeta').insert([
      {site_id: 1, description: 'This is a description for the Knexjs Site'},
      {site_id: 2, description: 'This is a description for the Bookshelfjs Site'}
    ]),

    knex('info').insert([
      {meta_id: 1, other_description: 'This is an info block for hasOne -> through test'},
      {meta_id: 2, other_description: 'This is an info block for an eager hasOne -> through test'}
    ]),

    knex('admins').insert([
      {username: 'test1', password: 'testpwd1', created_at: new Date(), updated_at: new Date()},
      {username: 'test2', password: 'testpwd2', created_at: new Date(), updated_at: new Date()}
    ]),

    knex('blogs').insert([
      {site_id: 1, name: 'Main Site Blog'},
      {site_id: 1, name: 'Alternate Site Blog'},
      {site_id: 2, name: 'Main Site Blog'},
      {site_id: 2, name: 'Alternate Site Blog'},
      {site_id: null, name: 'Orphan Blog Without a Site'}
    ]),

    knex('authors').insert([
      {site_id: 1, first_name: 'Tim', last_name: 'Griesser'},
      {site_id: 1, first_name: 'Bazooka', last_name: 'Joe'},
      {site_id: 2, first_name: 'Charlie', last_name: 'Brown'},
      {site_id: 2, first_name: 'Ron', last_name: 'Burgundy'},
      {site_id: 99, first_name: 'Anonymous', last_name: null}
    ]),
    ...
    knex('authors_posts').insert([
      ...
      {author_id: 3, post_id: 4},
      {author_id: 2, post_id: 1}
    ]),

    knex('tags').insert([{name: 'cool'}, {name: 'boring'}, {name: 'exciting'}, {name: 'amazing'}]),
    ...
    knex('comments').insert([
      {post_id: 1, name: '(blank)', email: 'test@example.com', comment: 'this is neat.'}
    ]),
