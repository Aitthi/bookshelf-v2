import { assignIn, clone, cloneDeep, defaultsDeep, drop, isString, map, omit, result } from './internal/lang';
...
import { NotFoundError, NoRowsUpdatedError, NoRowsDeletedError } from './errors';

import ModelBase from './base/model';

// any: attributes, options, relations, knex, the dynamic error subclassing and
// the `this` of bound non-arrow callbacks are all dynamic seams in this class.
...
const BookshelfModel = ModelBase.extend(
  {
     ...
     */
    hasOne(this: AnyVal, Target: AnyVal, foreignKey?: AnyVal, foreignKeyTarget?: AnyVal) {
     ...
     */
    hasMany(this: AnyVal, Target: AnyVal, foreignKey?: AnyVal, foreignKeyTarget?: AnyVal) {
     ...
     */
    belongsTo(this: AnyVal, Target: AnyVal, foreignKey?: AnyVal, foreignKeyTarget?: AnyVal) {
    ...
    belongsToMany(
      this: AnyVal,
      Target: AnyVal,
      joinTableName?: AnyVal,
      foreignKey?: AnyVal,
      otherKey?: AnyVal,
      foreignKeyTarget?: AnyVal,
      otherKeyTarget?: AnyVal
    ) {
     ...
     */
    morphOne(this: AnyVal, Target: AnyVal, name?: AnyVal, columnNames?: AnyVal, morphValue?: AnyVal) {
     ...
     */
    morphMany(this: AnyVal, Target: AnyVal, name?: AnyVal, columnNames?: AnyVal, morphValue?: AnyVal) {
     ...
     */
    morphTo(this: AnyVal, morphName: AnyVal) {
      ...
      if (args[1] == null || (Array.isArray(args[1]) && isString(args[1][0]))) {
        ...
        candidates = drop(Array.from(args), 2);
      } else {
        columnNames = null;
        candidates = drop(Array.from(args), 1);
      }

      candidates = map(candidates, (target: AnyVal) => {
        if (Array.isArray(target)) return target;

        // Set up the morphValue by default as the tableName
        return [target, result(target.prototype, 'tableName')];
      });
    ...
    through(
      this: AnyVal,
      Interim: AnyVal,
      throughForeignKey?: AnyVal,
      otherKey?: AnyVal,
      throughForeignKeyTarget?: AnyVal,
      otherKeyTarget?: AnyVal
    ) {
     ...
     */
    refresh(this: AnyVal, options: AnyVal = {}) {
      ...
      return this._doFetch(attributes, options).tap(() => {
        if (!options.silent) this._previousAttributes = cloneDeep(this.attributes);
      });
    },
     ...
     */
    fetchPage(this: AnyVal, options: AnyVal = {}) {
     ...
     *
     */
    fetch(this: AnyVal, options?: AnyVal) {
      return this._doFetch(this.attributes, options).tap(() => {
        this._previousAttributes = cloneDeep(this.attributes);
      });
    },

    // any: `this` in the bound non-arrow callbacks below is the BPromise-bound context (the model).
    _doFetch: BPromise.method(function (this: AnyVal, attributes: AnyVal, options?: AnyVal) {
      ...
      return (
        this.sync(options)
          ...
          .tap(function (this: AnyVal, response: AnyVal) {
          ...
          .return(this)
          .catch(function (this: AnyVal, err: AnyVal) {
            // Filtered catch: only handle NotFoundError instances; re-throw anything else.
            if (!(err instanceof this.constructor.NotFoundError)) throw err;
     ...
     */
    count(this: AnyVal, column?: AnyVal, options?: AnyVal) {
     ...
     */
    fetchAll(this: AnyVal, options?: AnyVal) {
      const collection = this.all();
      return collection
        .once('fetching', (__: AnyVal, columns: AnyVal, opts: AnyVal) => {
          /**
        ...
        .once('fetched', (__: AnyVal, response: AnyVal, opts: AnyVal) => {
          /**
     ...
     */
    load: BPromise.method(function (this: AnyVal, relations: AnyVal, options?: AnyVal) {
      const columns = this.format(assignIn({}, this.attributes));
      const withRelated = Array.isArray(relations) ? relations : [relations];
      return this._handleEager([columns], assignIn({}, options, {shallow: true, withRelated})).return(this);
    }),
     ...
     */
    save: BPromise.method(function (this: AnyVal, key: AnyVal, val?: AnyVal, options?: AnyVal) {
      let attrs: AnyVal;
      ...
      return BPromise.bind(this)
        .then(function (this: AnyVal) {
          return this.saveMethod(options);
        })
        .then(function (this: AnyVal, method: AnyVal) {
          ...
          if (method === 'insert' || options.defaults) {
            ...
            if (defaults) {
              attrs = (defaultsDeep as AnyVal)({}, attrs, this.attributes, defaults);
            }
          }
          ...
          return this.triggerThen(
            ...
            .then(function (this: AnyVal, resp: AnyVal) {
     ...
     */
    destroy: BPromise.method(function (this: AnyVal, options?: AnyVal) {
      options = options ? clone(options) : {};
      const sync = this.sync(options);
      options.query = sync.query;
      return BPromise.bind(this)
        ...
        .then(function (this: AnyVal, affectedRows: AnyVal) {
     ...
     */
    orderBy(this: AnyVal) {
      return Helpers.orderBy.apply(null, [this].concat(Array.from(arguments)) as AnyVal);
    },
    ...
    clone(this: AnyVal) {
      ...
      const cloned = BookshelfModel.__super__.clone.apply(this, arguments as AnyVal);
      if (this._knex != null) {
     ...
     * @returns Sync
     */
    sync(this: AnyVal, options: AnyVal) {
      // any: Sync is a Backbone-style constructor function (no TS construct signature).
      return new (Sync as AnyVal)(this, options);
    },
     ...
     * @private
     */
    _morphOneOrMany(this: AnyVal, Target: AnyVal, morphName: AnyVal, columnNames: AnyVal, morphValue: AnyVal, type: AnyVal) {
     ...
     */
    _handleResponse(this: AnyVal, response: AnyVal) {
     ...
     * @param {Object} Response from Knex query.
     */
    _handleEager(this: AnyVal, response: AnyVal, options: AnyVal) {
      return new EagerRelation([this], response, this).fetch(options);
    }
  },
  {
    extended(this: AnyVal, child: AnyVal) {
      /**
       ...
       */
      // any: dynamic error subclassing — replaces create-error(this.NotFoundError).
      // Preserves instanceof chain: new child.NotFoundError() instanceof this.NotFoundError === true
      child.NotFoundError = class extends (this as AnyVal).NotFoundError {};
       ...
       */
      child.NoRowsUpdatedError = class extends (this as AnyVal).NoRowsUpdatedError {};
       ...
       */
      child.NoRowsDeletedError = class extends (this as AnyVal).NoRowsDeletedError {};
    },

    fetchPage(this: AnyVal) {
      const model = this.forge();
      return model.fetchPage.apply(model, arguments as AnyVal);
    }
  }
);

// any: error classes are attached as static properties on the constructor function.
(BookshelfModel as AnyVal).NotFoundError = NotFoundError;
(BookshelfModel as AnyVal).NoRowsUpdatedError = NoRowsUpdatedError;
(BookshelfModel as AnyVal).NoRowsDeletedError = NoRowsDeletedError;
